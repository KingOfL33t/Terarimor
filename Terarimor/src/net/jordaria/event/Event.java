...
public abstract class Event {
	...
	public enum Result {
		 ...
		 */
		REJECT, /**
		 * * Neither deny nor allow the event. The server will proceed with
		 * its * normal handling.
		 */
		DEFAULT, /**
		 * * Allow / Force the event. The action indicated by the event
		 * will * take place if possible, even if the server would not normally
		 * allow * the action. Some actions may not be allowed.
		 */
		PERMIT;
	}
